...
namespace schema
{
	...
	/**
	 * [01.08.2026] Снимок ВСЕЙ схемы движка в файл: класс → поле → смещение → размер.
	 *
	 * ЗАЧЕМ. После каждого обновления игры часть полей исчезает, переезжает или меняет
	 * размер. Плагины об этом не знают: заголовки SchemaEntity пишутся руками и продолжают
	 * уверенно заявлять, что поле есть. Расхождение всплывает не сообщением, а падением
	 * сервера в чужом месте — и на его поиск уходят месяцы.
	 *
	 * Снимок делает расхождение видимым СРАЗУ: сняли до обновления, сняли после, сравнили —
	 * и получили точный список того, что сломалось, ещё до того, как зайдёт первый игрок.
	 *
	 * Формат намеренно простой и построчный, чтобы сравнивать обычным diff:
	 *     КЛАСС <имя> размер=<байт> полей=<n>
	 *       <смещение> <имя поля>
	 *
	 * @param pszPath куда писать. Каталог должен существовать.
	 * @return число выгруженных классов; 0 — снять не удалось (схема ещё не готова).
	 */
	int DumpAllToFile(const char* pszPath);
} // namespace schema
